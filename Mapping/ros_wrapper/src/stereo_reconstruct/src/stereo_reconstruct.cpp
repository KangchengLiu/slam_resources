        ...
        virtual void onInit() {
            ...
            if (approxSync) {
                approxSyncStereo_ = new message_filters::Synchronizer<MyApproxSyncStereoPolicy>(
                        MyApproxSyncStereoPolicy(queueSize), imageLeft_, imageRight_, cameraInfoLeft_,
                        cameraInfoRight_);
                approxSyncStereo_->registerCallback(
                ...
                exactSyncStereo_->registerCallback(
                        boost::bind(&StereoReconstruct::stereoCallback, this, _1, _2, _3, _4));
            }
...
//                        udisp.copyTo(mat_disp);
                        ...
                        disp_img = (unsigned char **)malloc(sizeof(unsigned char*));

                        *disp_img = (unsigned char*)malloc(sizeof(unsigned char) * mono_l.cols * mono_l.rows); // result depth map

                        stereo_bm(mono_l.data, mono_r.data, disp_img, mono_l.cols, mono_l.rows);
                        mat_disp = cv::Mat(mono_l.size(), CV_8UC1, *disp_img);
                    }
...
//                    cv::waitKey(10);
                ...
                *depth_frame_ = cv::Mat::zeros(mat_left.size(), depth_type);
                ...
                for (int h = 0; h < (int) pcl_cloud_->height; h++) {
                    for (int w = 0; w < (int) pcl_cloud_->width; w++) {
                        ...
                        if (std::isfinite(ptXYZ.x) && std::isfinite(ptXYZ.y) && std::isfinite(ptXYZ.z) &&
                            ptXYZ.z >= 0) {
                            pt.x = ptXYZ.x;
                            pt.y = ptXYZ.y;
                            pt.z = ptXYZ.z;
        ...
        void publishCloud(pcl::PointCloud<pcl::PointXYZRGB>::Ptr &pclCloud, const std_msgs::Header &header) {
            if (pclCloud->size() && (minDepth_ != 0.0 || maxDepth_ > minDepth_)) {
                ...
                filter.setFilterLimits(minDepth_,
                                       maxDepth_ > minDepth_ ? maxDepth_ : std::numeric_limits<float>::max());
