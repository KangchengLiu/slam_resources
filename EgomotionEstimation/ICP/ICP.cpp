...
string strFileCompare = "../compare.txt";

void GetCenterOfMass3D(const std::vector<Vector3d> &vvPts, Vector3d &center);
void ICP_SVD(const std::vector<Vector3d> &ptsA, const std::vector<Vector3d> &ptsB, Sophus::SE3<double> &se3AfromB);
void DrawTrajectory(vector<Sophus::SE3<double> > , vector<Sophus::SE3<double> >, Sophus::SE3<double>);

int main()
{
    ifstream fileCompare(strFileCompare);
    if(!fileCompare.is_open()){
        std::cout << "compare.txt open FAILED!" << std::endl;
       return -1; 
    }
    Eigen::Vector3d v3T;
    Eigen::Quaterniond q;
    ...
    std::vector<Sophus::SE3<double> > vse3_e;
    std::vector<Sophus::SE3<double> > vse3_g;
...
void ICP_SVD(const std::vector<Vector3d> &ptsA, const std::vector<Vector3d> &ptsB, Sophus::SE3<double> &se3AfromB)
{
    ...
    //compose SE3
    se3AfromB = Sophus::SE3<double>(m3R, v3T);
}

/*******************************************************************************************/
void DrawTrajectory(vector<Sophus::SE3<double> > posesA, vector<Sophus::SE3<double> > posesB, Sophus::SE3<double> se3AB) {
    if (posesA.empty()) {
